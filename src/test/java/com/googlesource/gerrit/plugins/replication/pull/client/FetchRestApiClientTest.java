...
import com.googlesource.gerrit.plugins.replication.pull.filter.AsyncRefsFilter;
...
public class FetchRestApiClientTest {
  ...
  AsyncRefsFilter asyncRefsFilter;
  ...
  public void setup() throws ClientProtocolException, IOException {
    ...
    when(config.getStringList("replication", null, "asyncRefs")).thenReturn(new String[0]);
    ...
    asyncRefsFilter = new AsyncRefsFilter(replicationConfig);
    objectUnderTest =
        new FetchRestApiClient(
            credentials, httpClientFactory, replicationConfig, asyncRefsFilter, pluginName, source);
  }
  ...
  public void shouldCallAsyncFetchForAllRefs()
      throws ClientProtocolException, IOException, URISyntaxException {
    when(config.getStringList("replication", null, "asyncRefs"))
