...
public abstract class PullReplicationSetupBase extends LightweightPluginDaemonTest {
  ...
  private void setReplicationSource(
    ...
    if (getGitTransportProtocol() == GitTransportProtocol.FILE) {
      List<String> replicaFileTransportUrls =
          buildReplicaURLs(
              replicaSuffixes, s -> gitPath.resolve("${name}" + s + ".git").toString());
      config.setStringList("remote", remoteName, "url", replicaFileTransportUrls);
    } else if (getGitTransportProtocol() == GitTransportProtocol.HTTP) {
      List<String> replicaHttpTransportUrls =
          buildReplicaURLs(replicaSuffixes, s -> adminRestSession.url() + "/${name}" + s + ".git");
      config.setStringList("remote", remoteName, "url", replicaHttpTransportUrls);
    }
  ...
  private List<String> buildReplicaURLs(
