...
public abstract class FetchRestApiClientBase {
  ...
  @Test
  public void shouldCallInitProjectWithConfigEndpoint() throws Exception {
    objectUnderTest.initProjectWithConfig(
        Project.nameKey("test_repo"),
        new URIish(api),
        eventCreatedOn,
    ...
    assertThat(httpPut.getURI().getPath())
        .isEqualTo(
            String.format(
                "%s/plugins/pull-replication/init-project-with-config/test_repo.git",
                urlAuthenticationPrefix()));
