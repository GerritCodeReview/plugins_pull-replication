...
public class FetchServiceTest {
  ...
  public void setup() throws URISyntaxException {
    ...
    when(fetchReplicationStateFactory.create(Mockito.any())).thenReturn(state);
    when(source.getURIs(projectName, null)).thenReturn(Lists.newArrayList(uri));
    when(sources.getAll()).thenReturn(Lists.newArrayList(source));
    when(source.schedule(projectName, OBJECT_ID_TO_FETCH, uri, state, true))
  ...
  public void shouldFetchRefUpdate() throws InterruptedException, ExecutionException {
    objectUnderTest.fetch(projectName, uri, OBJECT_ID_TO_FETCH);

    verify(source, Mockito.times(1)).schedule(projectName, OBJECT_ID_TO_FETCH, uri, state, true);
  }
  ...
  public void shouldMarkAllFetchTasksScheduled() throws InterruptedException, ExecutionException {
    ...
    verify(source, Mockito.times(1)).schedule(projectName, OBJECT_ID_TO_FETCH, uri, state, true);
    verify(state, Mockito.times(1)).markAllFetchTasksScheduled();
  }

  @Test
  public void shouldSkipFetchWhenUnknownSource()
      throws InterruptedException, ExecutionException, URISyntaxException {
    URIish unknownSourceUri = new URIish("file://unknown_host/repository_path/repo.git");
    objectUnderTest.fetch(projectName, unknownSourceUri, OBJECT_ID_TO_FETCH);

    verify(source, Mockito.never())
        .schedule(projectName, OBJECT_ID_TO_FETCH, unknownSourceUri, state, true);
  }
  ...
  public void shouldUpdateStateWhenInterruptedException()
    ...
    when(source.schedule(projectName, OBJECT_ID_TO_FETCH, uri, state, true)).thenReturn(future);

    try {
      objectUnderTest.fetch(projectName, uri, OBJECT_ID_TO_FETCH);
      fail("InterruptedException expected");
    } catch (InterruptedException e) {
      verify(fetchStateLog, Mockito.times(1))
  ...
  public void shouldUpdateStateWhenExecutionException()
    ...
    try {
      ...
      verify(fetchStateLog, Mockito.times(1))
          .error("An exception was thrown in fetch operation", e, state);
    }
  }
}
