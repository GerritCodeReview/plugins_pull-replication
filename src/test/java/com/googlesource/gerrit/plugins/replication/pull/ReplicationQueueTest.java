...
public class ReplicationQueueTest {
  ...
  public void shouldCallDeleteWhenReplicateProjectDeletionsTrue() throws IOException {
    when(source.isReplicateProjectDeletions()).thenReturn(true);
  ...
  @Test
  public void shouldNotCallDeleteWhenReplicateProjectDeletionsFalse() throws IOException {
    when(source.isReplicateProjectDeletions()).thenReturn(false);
  ...
  @Test
  public void shouldNotCallDeleteWhenProjectNotToFetch() throws IOException {
    when(source.isReplicateProjectDeletions()).thenReturn(true);
    when(source.getProjects()).thenReturn(ImmutableList.of("testProject"));
  ...
  @Test
  public void shouldCallDeleteWhenThereAreNoProject() throws IOException {
    when(source.isReplicateProjectDeletions()).thenReturn(true);
    when(source.getProjects()).thenReturn(ImmutableList.of());
    when(source.wouldFetchProject(any())).thenReturn(false);
    ...
    verify(source, times(1)).scheduleDeleteProject(any(), any(), any());
  }
