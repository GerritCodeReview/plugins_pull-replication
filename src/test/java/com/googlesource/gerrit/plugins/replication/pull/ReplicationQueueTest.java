...
public class ReplicationQueueTest {
  ...
  @Test
  public void shouldFallbackToCallSendObjectsWhenBatchApplyObjectsEndpointForbidden()
      throws Exception {
    // An un-upgraded target node doesn't recognise the batch-apply-objects URI in its own
    // Bearer-auth allow-list, so the request never reaches pull-replication's own routing and
    // is rejected by Gerrit core as an unauthenticated request (403).
    Event event = generateBatchRefUpdateEvent("refs/changes/01/1/1", "refs/changes/02/1/1");
    objectUnderTest.start();
    ...
    when(applyObjectsRefsFilter.match(any())).thenReturn(true);
    when(batchSendObjectsHttpResult.isSuccessful()).thenReturn(false);
    when(batchSendObjectsHttpResult.isForbidden()).thenReturn(true);
