...
public class BearerAuthenticationIT extends ActionITBase {
  ...
  public void shouldFetchRef() throws Exception {
    ...
    httpClientFactory
        .create(source)
        .execute(httpPost, assertHttpResponseCode(201), HttpClientContext.create());
  }
  ...
  public void shouldBe401WhenBearerTokenDoesNotMatch() throws Exception {
    ...
    final HttpPost httpPost = createRequest(sendObjectPayload);
    httpPost.addHeader(new BasicHeader(HttpHeaders.AUTHORIZATION, "Bearer " + "different-bearer-token"));
    httpClientFactory
            .create(source)
            .execute(httpPost, assertHttpResponseCode(401), HttpClientContext.create());
  }
  ...
  public void shouldBe401WhenBearerTokenCannotBeExtracted() throws Exception {
    ...
    final HttpPost httpPost = createRequest(sendObjectPayload);
    httpPost.addHeader(new BasicHeader(HttpHeaders.AUTHORIZATION, "some-value-cannot-be-extracted"));
    httpClientFactory
            .create(source)
            .execute(httpPost, assertHttpResponseCode(401), HttpClientContext.create());
  }
