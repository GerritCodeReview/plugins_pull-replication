...
public class FetchOneTest {
  ...
  @Test
  public void fetchWithoutDelta_shouldNotPassDeletedRefToFilterWhenMirroringIsDisabled()
      throws Exception {
    setupMocks(true);
    String LOCAL_REF = "refs/heads/local";
    String REMOTE_REF = "refs/heads/remote";
    Set<String> remoteRefs = Set.of(REMOTE_REF);
    setupRemoteConfigMock(List.of(ALL_REFS_SPEC), false);
    setupFetchRefsDatabaseMock(
        Map.of(LOCAL_REF, mock(Ref.class)), Map.of(REMOTE_REF, mock(Ref.class)));
    ReplicationFetchFilter mockFilter = setupReplicationFilterMock(remoteRefs);
  ...
  @Test
  public void fetchWithoutDelta_shouldPassDeletedRefToFilterWhenMirroringIsEnabled()
      throws Exception {
    setupMocks(true);
    String REMOTELY_DELETED_REF = "refs/heads/delete-remotely";
    ...
    setupRemoteConfigMock(List.of(ALL_REFS_SPEC), true);
    setupFetchRefsDatabaseMock(
        Map.of(REMOTELY_DELETED_REF, localRef), Map.of(REMOTE_REF, remoteRef));
    Set<String> expectedRefs = Set.of(REMOTELY_DELETED_REF, REMOTE_REF);
    ReplicationFetchFilter mockFilter = setupReplicationFilterMock(expectedRefs);

    objectUnderTest.run();

    verify(mockFilter).filter(TEST_PROJECT_NAME, expectedRefs);
  }
