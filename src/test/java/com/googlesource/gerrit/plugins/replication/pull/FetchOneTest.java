...
public class FetchOneTest {
  ...
  public void shouldRunAReplicationTaskForAllRefsIfDeltaIsEmpty() throws Exception {
    ...
    setupFetchFactory(Collections.emptyList());
  ...
  public void shouldFilterOutRefsFromFetchReplicationDelta() throws Exception {
    ...
    Set<String> refSpecs = Set.of(testRef, filteredRef);
    List<ReplicationState> states =
        Stream.concat(
                createTestStates(testRef, 1).stream(), createTestStates(filteredRef, 1).stream())
            .collect(Collectors.toList());
    setupFetchFactory(
        List.of(new FetchFactoryEntry.Builder().refSpecNameWithDefaults(testRef).build()),
        Optional.of(List.of(testRef)));
    ...
    when(mockFilter.filter(testProjectName, refSpecs)).thenReturn(Set.of(testRef));
    ...
    verify(states.get(1))
        .notifyRefReplicated(
            testProjectName, filteredRef, urIish, ReplicationState.RefFetchResult.FAILED, null);
  }
  ...
  private Fetch setupFetchFactory(List<FetchFactoryEntry> fetchFactoryEntries) throws Exception {
    return setupFetchFactory(fetchFactoryEntries, Optional.empty());
  }

  private Fetch setupFetchFactory(
    ...
    List<RefSpec> filteredRefSpecs =
        filteredRefs
            ...
            .orElse(refSpecs);
    Fetch mockFetch = setupFetchFactoryMock(filteredRefSpecs, refUpdateStates);
