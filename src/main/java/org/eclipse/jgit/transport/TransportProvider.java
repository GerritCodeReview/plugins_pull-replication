...
public class TransportProvider {
  ...
  public Transport open(Repository local, URIish uri)
    ...
    Transport transport =
        (bearerToken.isPresent() && TransportHttpWithBearerToken.canHandle(uri))
            ? provideTransportHttpWithBearerToken(local, uri)
            : provideOtherTransport(local, uri);
    return transport;
  }
  ...
  private Transport provideOtherTransport(Repository local, URIish uri)
      throws NotSupportedException, TransportException {
