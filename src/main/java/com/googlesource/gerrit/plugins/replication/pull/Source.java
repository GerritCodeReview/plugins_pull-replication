    ...
    synchronized (stateLock) {
      ...
      if (pendingFetchOp != null) {
        ...
        if (pendingFetchOp.isRetrying()) {
          ...
          stateLog.warn(String.format("[%s] Merging all refs to fetch from [%s] to the already retrying task [%s] for keeping its position into the replication queue", fetchOp.getTaskIdHex(), fetchOp.getURI(), pendingFetchOp.getTaskIdHex()),
          ...
          stateLog.warn(String.format("[%s] Merging the pending fetch from [%s] with task [%s] and rescheduling", pendingFetchOp.getTaskIdHex(), pendingFetchOp.getURI(), fetchOp.getTaskIdHex()),
                  fetchOp.getStatesAsArray());
        }
      }
