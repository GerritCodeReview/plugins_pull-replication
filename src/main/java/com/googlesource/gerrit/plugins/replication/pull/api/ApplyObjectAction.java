...
public class ApplyObjectAction implements RestModifyView<ProjectResource, Input> {
  ...
  public static class Revs {
    @SerializedName("object_blob")
    public Rev objectBlob;

    @SerializedName("tree_object")
    public Rev treeObject;

    public List<Rev> blobs;
  }
  ...
  public static class Rev {
    ...
    public Rev(int type, byte[] content) {
      this.type = type;
  ...
  private Response<?> applySync(Project.NameKey project, Input input)
    ...
    command.applyObject(
        project,
        input.refName,
        input.revs.objectBlob,
        input.revs.treeObject,
        input.revs.blobs,
        input.label);
