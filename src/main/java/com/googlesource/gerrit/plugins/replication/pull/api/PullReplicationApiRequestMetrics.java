...
public class PullReplicationApiRequestMetrics {
  private static final ThreadLocal<PullReplicationApiRequestMetrics> apiRequestMetrics =
      new ThreadLocal<>();
  ...
  public void start(HttpServletRequest req) {
    ...
    startTimeNanos =
        Optional.ofNullable(req.getHeader(HTTP_HEADER_X_START_TIME_NANOS)).map(Long::parseLong);
  }
