...
class PullReplicationGroupBackend extends AbstractGroupBackend {
  ...
  private static final AccountGroup.UUID INTERNAL_GROUP_UUID =
      AccountGroup.uuid("pullreplication:internal-user");
  private static final String INTERNAL_GROUP_NAME = "Pull-replication Internal User";
  private static final String NAME_PREFIX = "pullreplication/";
  private static final GroupDescription.Basic INTERNAL_GROUP_DESCRIPTION =
  ...
  public Collection<GroupReference> suggest(String name, ProjectState project) {
    return Arrays.asList(
        NAME_PREFIX.contains(name.toLowerCase())
            ? GroupReference.create(INTERNAL_GROUP_UUID, name)
