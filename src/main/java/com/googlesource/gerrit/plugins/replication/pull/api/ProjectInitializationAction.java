...
public class ProjectInitializationAction extends HttpServlet {
  ...
  protected void doPut(
    ...
    try {
      ...
      repLog.error("Init Project API FAILED for {}", gitRepositoryName);
      logger.atSevere().withCause(e).log(
          "Invalid request payload whilst creating project %s", gitRepositoryName);
      setResponse(httpServletResponse, SC_BAD_REQUEST, "Invalid request payload");
    } catch (IllegalArgumentException e) {
      repLog.error("Init Project API FAILED for {}", gitRepositoryName);
      logger.atSevere().withCause(e).log(
          "Ref-update with invalid input whilst creating project %s", gitRepositoryName);
      setResponse(httpServletResponse, SC_BAD_REQUEST, "Invalid request payload");
    } catch (RefUpdateException | MissingParentObjectException e) {
      repLog.error("Init Project API FAILED for {}", gitRepositoryName);
      logger.atSevere().withCause(e).log(
          "Error applying objects whilst creating project %s", gitRepositoryName);
      setResponse(httpServletResponse, SC_CONFLICT, e.getMessage());
    } catch (AuthException | PermissionBackendException e) {
      repLog.error("Init Project API FAILED for {}", gitRepositoryName);
      logger.atSevere().withCause(e).log(
          "User not authorized to create project %s", gitRepositoryName);
      setResponse(
          httpServletResponse,
          SC_FORBIDDEN,
          "User not authorized to create project " + gitRepositoryName);
    }
  }
