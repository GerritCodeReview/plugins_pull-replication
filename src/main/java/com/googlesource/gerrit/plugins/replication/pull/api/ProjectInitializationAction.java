...
public class ProjectInitializationAction extends HttpServlet {
  ...
  protected void doPut(
    ...
    String gitRepositoryName = Url.decode(path.substring(path.lastIndexOf('/') + 1));
    String projectName = gitRepositoryName.replace(".git", "");
    try {
      boolean initProjectStatus =
          (httpServletRequest.getContentLength() == 0)
              ? processRequest(gitRepositoryName)
              : processRequestWithConfiguration(httpServletRequest, gitRepositoryName, projectName);
  ...
  private boolean processRequestWithConfiguration(
      HttpServletRequest httpServletRequest, String gitRepositoryName, String projectName)
  ...
  private boolean initProject(String gitRepositoryName, boolean reindex)
      throws AuthException, PermissionBackendException {
