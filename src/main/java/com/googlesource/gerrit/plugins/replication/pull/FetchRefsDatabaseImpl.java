...
public class FetchRefsDatabaseImpl implements FetchRefsDatabase {

  private final TransportFactory transportFactory;

  @Inject
  public FetchRefsDatabaseImpl(TransportFactory transportFactory) {
  ...
  public Map<String, Ref> getRemoteRefsMap(Repository repository, URIish uri) throws IOException {
    try (Transport tn = transportFactory.open(repository, uri);
        FetchConnection fc = tn.openFetch()) {
      // TODO: Looks like refs pointing to blobs are not returned here.
      // This would be a problem for push replication too.
