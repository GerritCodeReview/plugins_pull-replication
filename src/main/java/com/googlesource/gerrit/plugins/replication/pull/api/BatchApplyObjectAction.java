...
public class BatchApplyObjectAction
  ...
  public Response<?> apply(ProjectResource resource, List<RevisionInput> inputs)
    ...
    List<RevisionInput> allRevisions = new ArrayList<>();
    for (RevisionInput input : inputs) {
      Response<RevisionInput> individualResponse = applySingleRef(resource, input);
      allRevisions.add(individualResponse.value());
    }

    // TODO is returning OK(200) appropriate?
    return Response.ok(allRevisions);
  }

  public Response<RevisionInput> applySingleRef(ProjectResource resource, RevisionInput input)
      throws RestApiException {
    try {
      ...
      if (Objects.isNull(input.getRevisionData())) {
        ...
        repLog.info(
            ...
            input.getRefName(),
            input.getRevisionData());
        return Response.withStatusCode(HttpServletResponse.SC_NO_CONTENT, input);
      }
