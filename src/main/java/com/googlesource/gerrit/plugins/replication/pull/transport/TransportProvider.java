...
/**
 * This class is responsible for setting bearer token header for Bearer Token Authentication
 * using {@link org.eclipse.jgit.transport.TransportHttp#setAdditionalHeaders(java.util.Map)}
 * method.
 */
@Singleton
public class TransportProvider {
  ...
  public Transport open(Repository local, URIish uri)
    ...
    if (tn instanceof TransportHttp && bearerToken.isPresent() && canHandle(uri)) {
      ((TransportHttp) tn)
          .setAdditionalHeaders(ImmutableMap.of(HDR_AUTHORIZATION, bearerToken.get()));
    } else {
