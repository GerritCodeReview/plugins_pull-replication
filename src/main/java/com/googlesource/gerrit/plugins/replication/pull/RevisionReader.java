...
public class RevisionReader {
  ...
  public Optional<RevisionData> read(
    ...
    try (Repository git = gitRepositoryManager.openRepository(project)) {
      ...
      List<ObjectId> parentObjectIds =
          getParentObjectIds(commit.getParents(), 0, Math.min(maxDepth, maxParentObjectIds));
  ...
  private List<ObjectId> getParentObjectIds(
      RevCommit[] commit, int parentsDepth, int maxParentObjectIds) {
    ...
    for (RevCommit revCommit : commit) {
      if (parentsDepth < maxParentObjectIds) {
        ...
        List<ObjectId> nestedParentObjectIds =
            getParentObjectIds(revCommit.getParents(), parentsDepth, maxParentObjectIds);
  ...
  private void verifySize(
    ...
    if (loader.isLarge()) {
      repLog.warn(
          ...
          projectName, refName, objectTypeToString(loader.getType()));
    ...
    if (totalRefSize > maxRefSize) {
      repLog.warn(
          ...
          projectName, refName, objectTypeToString(loader.getType()), totalRefSize, maxRefSize);
      throw new LargeObjectException(objectId);
    }
  }
