...
public class RevisionReader {
  ...
  /**
   * Reads and evaluates the git objects in this revision. The following are filtered out: - DELETE
   * changes - git submodule commits, because the git commit hash is not present in this repo. The
   * method keeps track of the total size of all objects it has processed, and verifies it is below
  ...
  private List<RevisionObjectData> readBlobs(
    ...
    for (DiffEntry diffEntry : diffEntries) {
      if (!ChangeType.DELETE.equals(diffEntry.getChangeType())
          && isNotGitSubmoduleCommit(diffEntry)) {
  ...
  private boolean isNotGitSubmoduleCommit(DiffEntry diffEntry) {
    return diffEntry.getNewMode() != FileMode.GITLINK;
  }
