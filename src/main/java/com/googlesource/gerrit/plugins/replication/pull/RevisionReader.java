...
public class RevisionReader {
  private static final Long DEFAULT_MAX_REF_SIZE_IN_BYTES = 10000L;
  ...
  public RevisionReader(GitRepositoryManager gitRepositoryManager, Config cfg) {
    ...
    this.maxRefSize =
        cfg.getLong("replication", "payloadMaxRefSize", DEFAULT_MAX_REF_SIZE_IN_BYTES);
  }
  ...
  public Optional<RevisionData> read(Project.NameKey project, String refName)
    ...
    try (Repository git = gitRepositoryManager.openRepository(project)) {
      ...
      repLog.trace(
          "Ref %s size for project %s is greater than configured 'payloadMaxRefSize'",
          refName, project);
