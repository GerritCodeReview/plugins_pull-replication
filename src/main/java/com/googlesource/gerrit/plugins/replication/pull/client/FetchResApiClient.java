...
public class FetchResApiClient {
  static String GERRIT_ADMIN_PROTOCOL_PREFIX = "gerrit+";

  public interface Factory {
    FetchResApiClient create(Source source);
  }
  ...
  public boolean callFetch(Project.NameKey project, String sha1, URIish targetUri) {
    String url =
        String.format(
    ...
    try {
      ...
      post.setEntity(
          new StringEntity(
              String.format("{\"url\":\"%s\", \"sha1\": \"%s\"}", toHttpUri(sourceUri), sha1),
              StandardCharsets.UTF_8));
      post.addHeader(new BasicHeader("Content-Type", "application/json"));
      return httpClient
          .execute(post, new HttpResponseHandler(), getContext(targetUri))
          .isSuccessful();
    } catch (IOException e) {
      return false;
    }
