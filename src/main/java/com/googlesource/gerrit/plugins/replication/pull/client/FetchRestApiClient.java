...
public class FetchRestApiClient implements ResponseHandler<HttpResult> {
  ...
  public HttpResult callSendObject(
      Project.NameKey project, String refName, byte[] object, URIish targetUri)
      throws ClientProtocolException, IOException {
    String url =
        String.format(
            "%s/a/projects/%s/pull-replication~applyObject",
            targetUri.toString(), Url.encode(project.get()));
    ...
    post.setEntity(
        new StringEntity(
            String.format(
                "{\"label\":\"%s\", \"ref_name\": \"%s\", \"object_blob\":\"%s\"}", instanceLabel, refName, new String(object)),
            StandardCharsets.UTF_8));
    post.addHeader(new BasicHeader("Content-Type", "application/json"));
    return httpClientFactory.create(source).execute(post, this, getContext(targetUri));
  }
