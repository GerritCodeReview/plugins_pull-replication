...
public class FetchRestApiClient implements FetchApiClient, ResponseHandler<HttpResult> {
  ...
  public HttpResult initProject(
      ...
      throws IOException {
    String url = formatInitProjectUrlWithRevisionData(uri.toString(), project);
    ...
    put.addHeader(new BasicHeader("Accept", MediaType.ANY_TEXT_TYPE.toString()));
    put.addHeader(new BasicHeader("Content-Type", MediaType.PLAIN_TEXT_UTF_8.toString()));
    HttpResult result = executeRequest(put, bearerTokenProvider.get(), uri);
    if (result.isNotFound()) {
      return initProject(project, uri);
    }
  ...
  private String formatInitProjectUrlWithRevisionData(String targetUri, Project.NameKey project) {
    return String.format(
        "%s/%splugins/%s/init-project/config/%s.git",
        targetUri, urlAuthenticationPrefix, pluginName, Url.encode(project.get()));
  }
