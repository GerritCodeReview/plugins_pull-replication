...
public class FetchRestApiClient implements FetchApiClient, ResponseHandler<HttpResult> {
  ...
  public HttpResult callBatchFetch(
      ...
      throws IOException {
    Map<Boolean, List<String>> refsPartitionedInAsyncAndSync = partitionRefsToAsyncAndSync(refsInBatch);
    boolean async = true;
    boolean sync = false;
    List<String> asyncRefs = refsPartitionedInAsyncAndSync.get(async);
    List<String> syncRefs = refsPartitionedInAsyncAndSync.get(sync);
