...
public class ApplyBatchObjectAction
  ...
  public Response<?> apply(ProjectResource resource, List<RevisionInput> inputs)
    ...
    for (RevisionInput input : inputs) {
      response = applySingleRef(resource, input);
    }
    // TODO: Handle response in a better way not just the last response code
    return response == null ? Response.ok() : response;
  }

  public Response<?> applySingleRef(ProjectResource resource, RevisionInput input)
      throws RestApiException {
    try {
