...
public class ReplicationQueue
  ...
  private CallFunction getCallFunction(
    ...
    try {
      List<BatchApplyObjectData> entries = refs.stream().map(
          ref -> {
            if (ref.isDelete()) {
              Optional<RevisionData> noRevisionData = Optional.empty();
              return new BatchApplyObjectData(ref.refName(), noRevisionData);
            }
            try {
              Optional<RevisionData> maybeRevisionData = revReaderProvider.get().read(project, ref.objectId(), ref.refName(), 0);
              return new BatchApplyObjectData(ref.refName(), maybeRevisionData);
            }
            catch (IOException e) {
              stateLog.error(
                  String.format(
      ...
      return ((source) -> callSendBatchObject(source, project, entries, eventCreatedOn, state));
  ...
  private boolean callSendBatchObject(
    ...
    List<BatchApplyObjectData> filteredRefs =
    ...
    for (String apiUrl : source.getApis()) {
      try {
        ...
        if (!resultSuccessful) {
          if (result.isParentObjectMissing()) {
            for (BatchApplyObjectData batchApplyObject : filteredRefs) {
              ...
              if ((RefNames.isNoteDbMetaRef(refName) || applyObjectsRefsFilter.match(refName))
                ...
                List<RevisionData> allRevisions = fetchWholeMetaHistory(project, refName, maybeRevisionData.get());
              ...
              throw new MissingParentObjectException(project, refName, source.getRemoteConfigName());
            }
          }

          if (result.isSendBatchObjectNotAvailable()) {
            resultSuccessful = true;
            for (BatchApplyObjectData batchApplyObjectData : filteredRefs) {
                boolean isDelete = batchApplyObjectData.getRevisionData().isEmpty();
                resultSuccessful &=
                    callSendObject(
                        source,
                        project,
                        ...
                        eventCreatedOn,
                        isDelete,
                        isDelete ? null : ImmutableList.of(batchApplyObjectData.getRevisionData().get()),
                        state);
              }
          }
        }
