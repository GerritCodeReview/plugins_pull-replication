...
public class ReplicationQueue
  ...
  private void fire(
    ...
    try {
      ...
      if (RefNames.isNoteDbMetaRef(refName)) {
        Revs revs = revReader.read(project, refName);
        fetchCallsPool
            .submit(
                () ->
                    sources
                        ...
                        .forEach(
                            source -> {
                              callSendObject(source, project, refName, revs, state);
                            }))
  ...
  private void callSendObject(
      Source source, Project.NameKey project, String refName, Revs revs, ReplicationState state) {
    if (source.wouldFetchProject(project) && source.wouldFetchRef(refName)) {
      for (String apiUrl : source.getApis()) {
        try {
          ...
          HttpResult result = fetchClient.callSendObject(project, refName, revs, uri);
