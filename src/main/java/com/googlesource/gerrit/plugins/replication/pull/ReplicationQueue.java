...
public class ReplicationQueue
  ...
  public void onGitReferenceUpdated(GitReferenceUpdatedListener.Event event) {
    if (isRefToBeReplicated(event.getRefName())) {
      ...
      fire(
          ...
          event.getRefName(),
          event.isDelete(), event.isNonFastForward());
    }
  }
  ...
  private void fire(String projectName, ObjectId objectId, String refName, boolean isDelete, boolean isForced) {
  ...
  private void fire(
      ...
      ObjectId objectId,
      String refName,
      boolean isDelete, boolean isForced,
  ...
  private CallFunction getCallFunction(
      ...
      ReplicationState state) {
    if (isDelete) {
      return ((source) -> callSendObject(source, project, refName, isDelete, isForced, null, state));
    }
    ...
    try {
      ...
      if (revisionData.isPresent()) {
        return ((source) ->
            callSendObject(
                source, project, refName, isDelete, isForced,Arrays.asList(revisionData.get()), state));
      }
            ...
            if (result.isParentObjectMissing()) {
              ...
              if (RefNames.isNoteDbMetaRef(refName) && revision.size() == 1) {
                ...
                repLog.info(
                    ...
                    refName,
                    allRevisions);
                return callSendObject(source, project, refName, isDelete, isForced, allRevisions, state);
              }
  ...
  private void fireBeforeStartupEvents() {
    ...
    for (ReferenceUpdatedEvent event : beforeStartupEventsQueue) {
      ...
      if (!eventsReplayed.contains(eventKey)) {
        ...
        fire(event.projectName(), event.objectId(), event.refName(), event.isDelete(), event.isForced());
