      ...
      try {
        ...
        if (!resultSuccessful && HttpResultUtils.isParentObjectMissing(result)) {
          ...
          for (BatchApplyObjectData batchApplyObject : filteredRefsBatch) {
            ...
            expandedBatch.add(BatchApplyObjectsData.create(refName, revisions));
          }
          ...
          String expandedBatchStr =
              expandedBatch.stream()
                  ...
                  .collect(Collectors.joining(","));
          repLog.info(
              "Sending whole history for {} refs as a single batch, one ref-update per ref:"
                  + " [{}]",
              expandedBatch.size(),
              expandedBatchStr);
          ...
          repLog.info(
              "Sending whole history for {} refs as a single batch COMPLETED, HTTP Result:"
                  + " {} - time:{} ms",
              expandedBatch.size(),
          ...
          if (!HttpResultUtils.isSuccessful(expandedResult)
              && HttpResultUtils.isNotFound(expandedResult)) {
            repLog.warn(
