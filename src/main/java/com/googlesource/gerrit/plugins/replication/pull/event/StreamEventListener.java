...
public class StreamEventListener implements EventListener {
  ...
  public void fetchRefsForEvent(Event event) throws AuthException, PermissionBackendException {
    ...
    if (event instanceof RefUpdatedEvent) {
      ...
      if (isApplyObjectsCacheHit(refUpdatedEvent)) return;
  ...
  private boolean isApplyObjectsCacheHit(RefUpdatedEvent refUpdateEvent) {
    ...
    Long refUpdateSuccededTimestamp = refUpdatesSucceededCache.getIfPresent(
            ApplyObjectsCacheKey.create(
                refUpdateAttribute.newRev,
                refUpdateAttribute.refName,
                refUpdateAttribute.project));
            
            return refUpdateSuccededTimestamp != null && refUpdateEvent.eventCreatedOn <= refUpdateSuccededTimestamp;
  }
}
