...
:	Use stream events to trigger pull-replication actions alongside the
	...
	Check the replication.streamEventsTopic parameter.

	replication.streamEventsTopic and replication.consumeStreamEvents
...
:	Topic to consumer stream events from.
	...
	NOTE: Messages will stay in the broker when a fetch fails throwing
	an exception. This will require some manual intervention to unblock
	the broker. Message consumption will be blocked until the fetch is not
	succesful or removed from the broker.
