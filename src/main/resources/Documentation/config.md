...
requirements and the set of supported options on both sides are different.
...
To use the REST API for replication notifications add the `apiUrl` to the remote section for which
...
To use the events-broker API for sending replication notifications configure the `eventBrokerTopic`:
